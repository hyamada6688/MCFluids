...
for j=0:63:(n_grid^2 - 1)
    xs_all(j+1, 1) = floor(j/n_grid);
    xs_all(j+1, 2) = mod(j, n_grid);
end

%xs_all = [20, 20; 40, 40; 60,60;80,80;100,100;120,120;140,140];
...
filename = "wossmall.gif"; % Specify the output file name
for idx = 1:nImages
    [A,map] = rgb2ind(im{idx},256);
    if idx == 1
        imwrite(A,map,filename,"gif","LoopCount",Inf,"DelayTime",0.03);
